    ...
    return (
        <Box as='div' bgGradient="linear(to bottom, #4529E6 50%, #F1F3F5 50%)" h="350" display="flex" justifyContent="center" alignItems="center" position="relative" bottom="35.5%" >
            <Box bg='#FDFDFD' w="75%" h="75%" p="9">
                ...
                <Text as='span' bg="#4529E6" p='5' borderRadius='100%' color="white">{mockUsers[3].name.split(" ").map((elem) => elem.charAt(0)).join("").slice(0, 2).toUpperCase()}</Text>
                <Box as='div' display="flex">
                    {/* <Text mt="8" >{user?.name.toUpperCase()}</Text> */}
                    <Text mt="8" >{mockUsers[3].name.toUpperCase()}</Text>
                ...
                <Text mt="5">{mockUsers[3].description}</Text>
